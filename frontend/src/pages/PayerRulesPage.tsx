    ...
    <div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-1">
          <div className="glass-card rounded-lg" data-testid="payer-list">
            <div className="border-b border-glass px-4 py-3">
              <h3 className="text-sm font-semibold text-slate-100">Payers</h3>
            </div>
            {payers.length === 0 ? (
              <p className="p-4 text-center text-sm text-slate-500" data-testid="payer-list-empty">
              ...
              <ul className="divide-y divide-glass">
                {payers.map((payer) => (
                  <li
                    ...
                    className={`flex cursor-pointer items-center justify-between px-4 py-3 transition-colors hover:bg-slate_d-700 ${
                      selectedPayer?.id === payer.id ? "bg-[rgba(99,102,241,0.1)]" : ""
                    }`}
        ...
        <div className="lg:col-span-2">
          {!selectedPayer ? (
            <div className="glass-card rounded-lg py-12 text-center text-sm text-slate-500" data-testid="no-payer-selected">
            ...
            <div>
              ...
              {saveSuccess && (
                <div className="mb-4 rounded-md border border-mint-600/30 bg-mint-600/10 p-3 text-sm text-mint-500" data-testid="save-success">
                  {saveSuccess}
                </div>
              )}
              {saveError && (
                <div className="mb-4 rounded-md border border-coral-600/30 bg-coral-600/10 p-3 text-sm text-coral-500" data-testid="save-error">
                  {saveError}
                </div>
              )}
              ...
              {/* New rule form */}
              {showNewRule && (
                <div className="mb-4 rounded-lg border border-accent-700/30 bg-accent-700/5 p-4" data-testid="new-rule-form">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="col-span-2">
                      ...
                      <textarea
                        ...
                        rows={4}
                        className="dark-input w-full font-mono text-xs"
                      />
                    </div>
                  </div>
              ...
              {/* Rules error */}
              {rulesError && (
                <div className="mb-4 rounded-md border border-coral-600/30 bg-coral-600/10 p-3 text-sm text-coral-500" data-testid="rules-error">
                  {rulesError}
                </div>
              )}
              ...
              {rulesLoading ? (
                <div className="flex items-center justify-center py-8">
                  <div className="h-5 w-5 animate-spin rounded-full border-4 border-accent-700 border-t-transparent" />
                </div>
              ) : rules.length === 0 ? (
                <div className="glass-card rounded-lg py-8 text-center text-sm text-slate-500" data-testid="rules-empty">
                    ...
                    >
                      {editingRule?.id === rule.id ? (
                        ...
                        <div>
                          <div className="flex items-start justify-between">
                            <div>
                              <div className="flex items-center gap-2">
                                <span className="rounded bg-slate_d-600 px-2 py-0.5 text-xs font-medium text-slate-300">
                                  {rule.rule_type}
                                </span>
                          ...
                          <pre className="mt-2 max-h-32 overflow-auto rounded-md bg-slate_d-900 p-2 text-xs text-slate-400 font-mono">
